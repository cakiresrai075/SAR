...
# CONFIGURATION
# =========================
WEIGHTS = "/content/drive/MyDrive/SAR/aircraft_v5X_1024/weights/best.pt"
IMG_DIR = "/content/drive/MyDrive/SAR/YOLO_AIRCRAFT/YOLO_AIRCRAFT/images/test"
OUT_DIR = "/content/drive/MyDrive/CAM_OUT/AIRCRAFT_GRADCAM"
    ...
    def get_cam_weights(
        ...
        # 3D image: [B, C, D, H, W]
            ...
            raise ValueError(
                ...
                "Shape should be 4 (2D image) or 5 (3D image)."
            )
...
def run():
    ...
    for name, mod in zip(target_names, target_modules):
        gradcam_dict[name] = GradCAM(model, [mod])
    ...
    for idx, pth in enumerate(imgs, 1):
        ...
        try:
            ...
            for layer_name in target_names:
                # Compute Grad-CAM
                cam = gradcam_dict[layer_name](x, targets)
