...
OUT_DIR = "/content/drive/MyDrive/CAM_OUTT/AIR/AIRCRAFT_LAYERCAM"
...
class LayerCAM:
    ...
    def __call__(self, input_tensor, target_fn):
        ...
        # LayerCAM: ReLU(grads) * activations -> spatial agirlikli
        spatial_weighted = np.maximum(grads, 0) * activations  # [1, C, H, W]
        cam = spatial_weighted.sum(axis=1)                      # [1, H, W]
        return cam
...
def run():
    ...
    print(f"[INFO] Block type  : {net.model[TARGET_LAYER_ID].__class__.__name__}")
    print(net.model[TARGET_LAYER_ID])  # blogun tam yapisi
