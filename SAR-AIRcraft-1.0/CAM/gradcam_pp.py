...
# Target Layer: Last Conv (otomatik bulunur)
# Target Function: LogSumExp (smooth-max)
# ============================================================
...
OUT_DIR = "/content/drive/MyDrive/CAM_OUTT/AIR/AIRCRAFT_GRADCAMPP"
...
LSE_TAU = 0.2   # LogSumExp tau: kucuk -> daha max-like. Onerilen: 0.05~0.20
...
def find_last_conv(net):
    """Modeldeki son Conv2d katmanini otomatik bulur."""
    last = None
    last_name = None
...
# =========================
# TARGET – LogSumExp (Grad-CAM++ icin)
# =========================
class YOLOGlobalTargetLSE:
    """
    Grad-CAM++ icin LogSumExp hedef fonksiyonu.
    Gradyanlarin cökmesini önler ve kücük nesnelere odaklanmayi arttirir.
    Mutlaka skalar döndürmeli -> reshape(-1) ile flatten edilir.
    """
    ...
    def __call__(self, pred):
        ...
        # Flatten -> logsumexp skalar dönürür
...
class GradCAMPlusPlus:
    ...
    def __call__(self, input_tensor, target_fn):
        ...
        # Grad-CAM++ agirlik hesabi
        grads_power_2 = grads ** 2
        grads_power_3 = grads_power_2 * grads

        sum_activations = np.sum(activations, axis=(2, 3))  # [1, C]
        eps = 1e-7
        ...
        weights = np.maximum(grads, 0) * aij       # [1, C, H, W]
        weights = np.sum(weights, axis=(2, 3))      # [1, C]
...
def run():
    ...
    # Son Conv katmanini bul
    last_conv, last_name = find_last_conv(net)
    if last_conv is None:
        raise RuntimeError("Son Conv2d katmani bulunamadi!")
    print(f"[INFO] Last conv layer: {last_name}")
    print(f"[INFO] Last conv     : {last_conv}")  # katmanin tam yapisi

    gradcampp = GradCAMPlusPlus(model, last_conv)
    target_fn = YOLOGlobalTargetLSE(tau=LSE_TAU)
